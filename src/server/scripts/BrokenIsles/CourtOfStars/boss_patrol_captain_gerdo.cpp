/*
 * Copyright (C) 2019 MagiLife-Core. 
 * in Collaboration with WoW-CCN
 *
...
public:
    ...
    struct boss_patrol_captain_gerdo_AI : public BossAI
    {
        ...
        void CheckBeacons()
        {
            ...
            for (auto & it : beacons)
            {
                if (it->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK))
                    beaconsActivated++;
            }
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_cos_flask_of_the_solemn_night_AI(creature);
    }
};
...
public:
    ...
    class spell_gerdo_arcane_lockdown_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gerdo_arcane_lockdown_AuraScript);
        ...
        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_gerdo_arcane_lockdown_AuraScript::HandleOnApply, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
            OnEffectApply += AuraEffectApplyFn(spell_gerdo_arcane_lockdown_AuraScript::HandleOnApply, EFFECT_1, SPELL_AURA_MOD_DECREASE_SPEED, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_gerdo_arcane_lockdown_AuraScript();
    }
};
...
public:
    ...
    public:
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gerdo_streetsweeper_dmg_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };
