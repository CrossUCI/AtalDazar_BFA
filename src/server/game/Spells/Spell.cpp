...
void SpellCastTargets::OutDebug() const
{
    ...
    if (m_orientation.is_initialized())
        TC_LOG_DEBUG("spells", "orientation: %f", m_orientation);
    if (m_mapId.is_initialized())
       ...
       // Unit has to be target only of aura effect, both caster and target have to be players, target has to be other than unit target
        ...
        if (m_originalCaster)
        {
            ...
            m_spellAura = Aura::TryRefreshStackOrCreate(m_spellInfo, m_castId, effectMask, unit,
                ...
                m_CastItem, ObjectGuid::Empty, &refresh, resetPeriodicTimer, ObjectGuid::Empty, m_castItemLevel, this);
            if (m_spellAura)
            {
...
bool WorldObjectSpellNearbyTargetCheck::operator()(WorldObject* target)
{
    ...
    if (_caster != target && dist < _range && WorldObjectSpellTargetCheck::operator ()(target))
    {
        _range = dist;
        return true;
    }
