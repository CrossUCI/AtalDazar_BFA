...
float Unit::GetUnitCriticalChance(WeaponAttackType attackType, Unit const* victim) const
{
    ...
    AuraEffectList const& modCritChanceAbovePctAuras = GetAuraEffectsByType(SPELL_AURA_MOD_CRIT_CHANCE_ON_TARGETS_ABOVE_HEALTH_PCT);
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, int32* baseAmount /*= nullptr*/, Item* castItem /*= nullptr*/, ObjectGuid casterGUID /*= ObjectGuid::Empty*/, bool resetPeriodicTimer /*= true*/, ObjectGuid castItemGuid /*= ObjectGuid::Empty*/, int32 castItemLevel /*= -1*/, Spell* spell /*= nullptr*/)
{
...
void Unit::SetSpeedRate(UnitMoveType mtype, float rate)
{
    ...
    if (Player* playerMover = GetCharmerOrOwnerPlayerOrPlayerItself()) // unit controlled by a player.
    {
...
int32 Unit::CalcSpellDuration(SpellInfo const* spellProto, Spell const* spell)
{
    uint32 comboPoints = 0;
    if (spell)
        if (SpellPowerCost const* powerCost = spell->GetPowerCost(POWER_COMBO_POINTS))
